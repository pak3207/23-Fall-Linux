...
ssize_tdevice_write(struct file *filp, const char *buf, size_tcount, loff_t*f_pos) {
copy_from_user(device_buf, buf, count);return count;
}

ssize_tdevice_read(struct file *filp, char *buf, size_tcount, loff_t*fpos) {
...
return -1;
	...
	if(device_buf!= NULL)memset(device_buf, 0, MAX_SIZE);
	return 0;
}
...
if (device_buf!= NULL)kfree(device_buf);
}
